...
public class VacancyRepoImpl implements VacancyRepo{
    ...
    public Vacancy saveOrUpdate(Vacancy vacancy) {
        ...
        try {
            ...
            if (existingVacancy.getId() != -1L) {
                stmt.setLong(10, existingVacancy.getId());
            }
            stmt.executeUpdate();
    ...
    public Vacancy findByHhid(String hhid) {
        try {
            PreparedStatement stmt = connection.prepareStatement( "SELECT * FROM vacancy WHERE hhid = ?" );
    ...
    public Vacancy findById(long vacancyId) {
        try {
            PreparedStatement stmt = connection.prepareStatement("SELECT * FROM vacancy WHERE id = ?");
    ...
    private PreparedStatement insertStatementDriverManager() throws Exception {
        return connection.prepareStatement("""
                INSERT INTO vacancy (hhid, name, salary_from, salary_to, gross, url, alternate_url, employer_id, archived) VALUES (?,?,?,?,?,?,?,?,?)
    ...
    private PreparedStatement updateStatementDriverManager() throws Exception {
        return connection.prepareStatement("""
                UPDATE vacancy SET hhid=?,name=?, salary_from=?, salary_to=?, gross=?, url=?, alternate_url=?, employer_id=?, archived=? WHERE id=?
                   """);
    }
}
